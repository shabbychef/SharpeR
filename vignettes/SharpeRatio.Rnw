...
Let \pvschol be a Cholesky factor of \pvsig: $\pvsig = \ogram{\pvschol}$. We can rewrite
\begin{equation*}
\begin{split}
\frac{\psnrsopt}{\psnropt} 
&=
\frac{\trAB{\pvmu}{\sportwopt}}{\sqrt{\qform{\pvsig}{\sportwopt}\qform{\minv{\pvsig}}{\pvmu}}},\\
&=
\frac{\trAB{\wrapNeParens{\minv{\pvschol}\pvmu}}{\tr{\pvschol}\sportwopt}}{\sqrt{\gram{\wrapNeParens{\tr{\pvschol}\sportwopt}}\gram{\wrapNeParens{\minv{\pvschol}\pvmu}}}},\\
&= \frac{\trAB{\wrapNeParens{\minv{\pvschol}\pvmu}}{\tr{\pvschol}\sportwopt}}{\norm{\minv{\pvschol}\pvmu}\norm{\tr{\pvschol}\sportwopt}}.
\end{split}
\end{equation*}

Now let \Pmat be an orthogonal matrix which rotates $\minv{\pvschol}\pvmu$ onto the first coordinate:
$\Pmat\minv{\pvschol}\pvmu = \norm{\minv{\pvschol}\pvmu}\basev[1]$. 
Insert $\tr{\Pmat}\trminv{\Pmat}$ into the numerator:
\begin{equation*}
\begin{split}
1-\hcut = \frac{\psnrsopt}{\psnropt} 
&= \frac{\trAB{\wrapNeParens{\Pmat\minv{\pvschol}\pvmu}}{\trminv{\Pmat}\tr{\pvschol}\sportwopt}}{\norm{\minv{\pvschol}\pvmu}\norm{\tr{\pvschol}\sportwopt}},\\
&= \frac{\trAB{\wrapNeParens{\norm{\minv{\pvschol}\pvmu}\basev[1]}}{\trminv{\Pmat}\tr{\pvschol}\sportwopt}}{\norm{\minv{\pvschol}\pvmu}\norm{\trminv{\Pmat}\tr{\pvschol}\sportwopt}},\\
&= \frac{\trAB{\basev[1]}{\trminv{\Pmat}\tr{\pvschol}\sportwopt}}{\norm{\trminv{\Pmat}\tr{\pvschol}\sportwopt}}.
\end{split}
\end{equation*}

Now suppose that instead of observing \vreti, one had observed 
$\vretj = \Pmat\minv{\pvschol}\vreti$.  The mean and covariance of
\vreti are \pvmu and \pvsig; the mean and covariance of \vretj are
$\norm{\minv{\pvschol}\pvmu}\basev[1]$ and 
$\qoform{\pvsig}{\wrapNeParens{\Pmat\minv{\pvschol}}}$.
Recall that \pvschol is the Cholesky factor of \pvsig, so
$\qoform{\pvsig}{\minv{\pvschol}} = \eye$ and
$$
\norm{\minv{\pvschol}\pvmu} =
\sqrt{\qform{\trminv{\pvschol}\minv{\pvschol}}{\pvmu}} =
\sqrt{\qform{\minv{\pvsig}}{\pvmu}} = \psnropt.$$
Thus the mean and covariance of \vretj are 
$\psnropt\basev[1]$ and $\eye$.

The (sample) optimal portfolio on the observations of \vretj corresponding to
those of \vreti is
$$
\sportw[y,*] = \minv{\wrapNeParens{\Pmat\minv{\pvschol}\svsig\trminv{\pvschol}\tr{\Pmat}}}\Pmat\minv{\pvschol}\svmu 
= \trminv{\Pmat}\tr{\pvschol}\minv{\svsig}\svmu 
= \trminv{\Pmat}\tr{\pvschol}\sportw[*].
$$
So we can rewrite the haircut (in terms of \vreti) as 
\begin{equation}
1-\hcut = \frac{\trAB{\basev[1]}{\sportw[y,*]}}{\norm{\sportw[y,*]}}.
\end{equation}

So \WLOG it suffices to study the case where one observes \vretj, forms the Markowitz portfolio and experiences
some haircut.  But the population parameters associated with 
\vretj are simpler to deal with.

\subsubsection{A Simple Approximation for Gaussian Returns}
