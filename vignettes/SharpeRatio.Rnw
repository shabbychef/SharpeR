...
\end{equation}
is an unbiased since $\pvsm = \E{\gram{\avreti}}$.
...
= - \qoform{\wrapParens{\minv{\Mtx{A}} \kron \minv{\Mtx{A}}}}{\Elim}.
\end{equation}
...
\rightsquigarrow \\
\normlaw{0,\qform{\pvvar}{\wrapBracks{\qoform{\wrapParens{\minv{\svsm} \kron
\minv{\svsm}}}{\Elim}}}}.
\label{eqn:mvclt_isvsm}
\end{multline}
%\end{equation}
It only remains to estimate \pvvar by some consistent estimator, call
it \svvar. 
...
More elaborate covariance estimators can also be used to deal with
violations of the \iid assumptions.

\nocite{magnus1999matrix,magnus1980elimination}
\nocite{BrittenJones1999}

Empirically, the implicit tests for significance based on this computation
give results nearly identical to the procedure 
of Britten-Jones, as shown below. \cite{BrittenJones1999} 
What differences exist are likely
due to differences in degrees of freedom accounting.
...
# t-stat via inverse second moment trick
ism.ts <- function(rets) {
...
\end{equation*}
leaning heavily on the Matrix Cookbook for 
the derivatives. \cite{petersen2006matrix,magnus1999matrix}
This is solved by $\pvsm[\txtMLE] = \svsm$, and thus the
unbiased sample estimator is also the MLE.
