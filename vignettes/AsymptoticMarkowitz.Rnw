...
This follows the so-called `numerator layout'. 
For matrices \Mtx{X} and \Mtx{Y}, let
\begin{equation*}
...
= - \EXD{\wrapParens{\trminv{\Mtx{A}}\kron\minv{\Mtx{A}}}}.
...
For the symmetric case, use the chain rule:
\begin{equation*}
\dbyd{\fvech{\minv{\Mtx{A}}}}{\fvech{\Mtx{A}}} 
= \dbyd{\fvech{\minv{\Mtx{A}}}}{\fvec{\minv{\Mtx{A}}}} 
  \dbyd{\fvec{\minv{\Mtx{A}}}}{\fvec{\Mtx{A}}}
  \dbyd{\fvec{\Mtx{A}}}{\fvech{\Mtx{A}}}
= \EXD{\dbyd{\fvec{\minv{\Mtx{A}}}}{\fvec{\Mtx{A}}}},
\end{equation*}
where the last equation follows because the definitions
$\fvech{\Mtx{X}} = \Elim\fvec{\Mtx{X}}$ 
and 
$\fvec{\Mtx{X}} = \Dupp\fvech{\Mtx{X}}$ imply
$$
\dbyd{\fvech{\minv{\Mtx{A}}}}{\fvec{\minv{\Mtx{A}}}} = \Elim,
\quad\mbox{and}\quad
\dbyd{\fvec{\Mtx{A}}}{\fvech{\Mtx{A}}} = \Dupp.
$$
\end{proof}%UNFOLD
...
\drbydr{\fvec{\drbydr{\log\FOOpdf{}{\svsm}{\pvsm}}{\minv{\pvsm}}}}{\fvec{\minv{\pvsm}}},\\
 &= - \half[\ssiz] \dbyd{\pvsm - \svsm}{\fvec{\minv{\pvsm}}},\\
...
To get the Fisher Information with respect to \fvech{\minv{\pvsm}}, apply the
change of variables formula, using the fact that 
$\dbyd{\fvec{\Mtx{A}}}{\fvech{\Mtx{A}}} = \Dupp.$
\end{proof}%UNFOLD
%\begin{corollary}%FOLDUP
...
\sqrt{\ssiz}\wrapParens{\fvech{\minv{\svsm}} - \fvech{\minv{\pvsm}}} 
\rightsquigarrow 
\normlaw{0,2 \minv{\wrapParens{\qform{\wrapParens{\AkronA{\pvsm}}}{\Dupp}}}}.
...
\begin{example}
To sanity check \theoremref{inv_distribution_gaussian}, 
...
respectively. By the delta method, the asymptotic variances of the elements
of \fvech{\minv{\pvsm}} can be computed as
\begin{equation*}
\begin{split}
\VAR{1 + \frac{\smu^2}{\ssig^2}} 
&= \wrapParens{\frac{2\pmu}{\psig^2}}^2\psig^2
 + \wrapParens{- \frac{\pmu^2}{\psig^4}}^2 2\psig^4
= \frac{4\pmu^2}{\psig^2} + \frac{2\pmu^4}{\psig^4},\\
\VAR{- \frac{\smu}{\ssig^2}} 
&= \wrapParens{-\frac{1}{\psig^2}}^2\psig^2 
 + \wrapParens{- \frac{\pmu}{\psig^4}}^2 2\psig^4
= \frac{1}{\psig^2} + \frac{2\pmu^2}{\psig^4},\\
\VAR{\frac{1}{\ssig^2}} 
&= \wrapParens{- \frac{1}{\psig^4}}^2 2\psig^4
= \frac{2}{\psig^4}.
\end{split}
\end{equation*}

Now compute the same quantities via \theoremref{inv_distribution_gaussian}, 
