...
\subsubsection{Hedging with Portfolio Constraints}

In the hedged portfolio optimization problem above, the optimal portfolio
...
Suppose then that the portfolio restriction is $\zerJc\portw = 0$, for
a given full-rank $\wrapParens{\nlatf - \nlatfzer} \times \nlatf$
matrix \zerJc. Let \zerJ be the complement of \zerJc. That is,
$\zerJc \tr{\zerJ} = \mzero$ and $\gram{\zerJ} = \eye$.
...
is $\fvola[i]\sqrt{\qiform{\pvsig}{\pvmu}}$, thus the model names.
