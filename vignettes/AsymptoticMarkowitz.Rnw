...
Define
\begin{equation}
\fsymd{\Mtx{B}} \defeq \Mtx{B} + \tr{\Mtx{B}} - \diag{\Mtx{B}},
\end{equation}
where \diag{\Mtx{B}} is the diagonal matrix with the same diagonal
as \Mtx{B}.

Then, for symmetric, invertible matrix \Mtx{A}, 
\begin{equation}
\dbyd{\fvec{\minv{\Mtx{A}}}}{\fvec{\Mtx{A}}} 
= \fsymd{- \AkronA{\minv{\Mtx{A}}}} 
= - 2 \wrapParens{\AkronA{\minv{\Mtx{A}}}}
+ \diag{\AkronA{\minv{\Mtx{A}}}},
\label{eqn:deriv_vech_matrix_inverse}
\end{equation}
and thus
\begin{equation*}
\dbyd{\fvech{\minv{\Mtx{A}}}}{\fvech{\Mtx{A}}} = 
\qoElim{\fsymd{- \AkronA{\minv{\Mtx{A}}}}}.
\end{equation*}
\label{lemma:deriv_vech_matrix_inverse}
\end{lemma}%UNFOLD
\begin{proof}%FOLDUP
For general matrix \Mtx{A}, the following is 
known, \cite{facklernotes,magnus1999matrix}
\begin{equation}
\dbyd{\fvec{\minv{\Mtx{A}}}}{\fvec{\Mtx{A}}} 
= - \wrapParens{\trminv{\Mtx{A}}\kron\minv{\Mtx{A}}}.
\label{eqn:deriv_matrix_inverse}
\end{equation}
Note how this generalizes the derivative in the 
scalar case:
$\dbyd{x^{-1}}{x} = - \wrapParens{x^{-1} x^{-1}}.$
For \emph{symmetric} \Mtx{A}, this relation has to 
be adjusted.  The function \fsymd{\cdot}
takes the general derivative to the form for a derivative
with respect to a symmetric matrix.  \cite[\cf section 2.8.2]{petersen2012matrix}
Thus, for symmetric \Mtx{A}, 
\begin{equation}
\dbyd{\fvec{\minv{\Mtx{A}}}}{\fvec{\Mtx{A}}} 
= \fsymd{- \wrapParens{\trminv{\Mtx{A}}\kron\minv{\Mtx{A}}}}
= - 2 \wrapParens{\AkronA{\minv{\Mtx{A}}}} + \diag{\AkronA{\minv{\Mtx{A}}}}.
\label{eqn:deriv_symmetric_matrix_inverse}
\end{equation}

The non-redundant parts of this can then be expressed via the
Elimination matrix,
\begin{equation*}
\dbyd{\fvech{\minv{\Mtx{A}}}}{\fvech{\Mtx{A}}} 
= \qoElim{\dbyd{\fvec{\minv{\Mtx{A}}}}{\fvec{\Mtx{A}}}}.
\end{equation*}
\end{proof}%UNFOLD
...
\rightsquigarrow \\
\normlaw{0,\qform{\pvvar}{\wrapBracks{\qoform{\fsymd{-\AkronA{\minv{\pvsm}}}}{\Elim}}}},
\label{eqn:mvclt_isvsm}
\end{multline}
%\end{equation}
where \pvvar is the variance of $\fvech{\ogram{\avreti}}$, and
$\fsymd{\Mtx{B}} \defeq \Mtx{B} + \tr{\Mtx{B}} - \diag{\Mtx{B}}$.
...
\end{split}
\end{equation}
where the matrix calculus follows from standard 
results. \cite{petersen2012matrix}
\nocite{magnus1999matrix,petersen2012matrix}
Note we need not take symmetry into account in the derivative, since
it is possible that the MLE could be non-symmetric.
Thus \svsm is a root to the equation, and the likelihood is maximized by 
$\pvsm[\txtMLE] = \svsm$.
\end{proof}%UNFOLD
...
\begin{equation}
\FishIf[\ssiz]{\minv{\pvsm}} 
= \half[\ssiz] \fsymd{\AkronA{\pvsm}}
= \ssiz \wrapParens{\AkronA{\pvsm}} -
\half[\ssiz] \diag{\AkronA{\pvsm}},
\label{eqn:itheta_fish_inf_eqn}
\end{equation}
and thus the Fisher Information of $\fvech{\minv{\pvsm}}$ is
\begin{equation}
\FishIf[\ssiz]{\fvech{\minv{\pvsm}}} 
= \ssiz \qoElim{\wrapParens{\AkronA{\pvsm}}} -
\half[\ssiz] \qoElim{\diag{\AkronA{\pvsm}}}.
...
\drbydr[2]{\log\FOOpdf{}{\svsm}{\pvsm}}{\minv{\pvsm}}
 & \defeq
\drbydr{\fvec{\drbydr{\log\FOOpdf{}{\svsm}{\pvsm}}{\minv{\pvsm}}}}{\fvec{\minv{\pvsm}}},\\
 &= - \half[\ssiz] \fsymd{\AkronA{\pvsm}}.
\end{split}
\end{equation*}
...
\sqrt{\ssiz}\wrapParens{\fvech{\minv{\svsm}} - \fvech{\minv{\pvsm}}} 
\rightsquigarrow 
\normlaw{0,2 \minv{\wrapParens{\qoElim{\wrapParens{\fsymd{\AkronA{\pvsm}}}}}}}.
...
\rightsquigarrow 
\normlaw{0,\minv{\wrapBracks{\FishIf[\ssiz]{\fvech{\minv{\pvsm}}}}}},
\end{equation}
...
\begin{example}
Consider the one-dimensional case. In this case, \fvech
foo text.  \cite{spiegel2007schaum}
