...
onto the first coordinate.\sidenote{This can be done by
taking a lower Cholesky factor, then a rotation.} 
...
qhcut2 <- function(p, df1, df2, zeta.s, lower.tail=TRUE) {
	atant <- atan((1/sqrt(df1-1)) * qt(p,df=df1-1,ncp=sqrt(df2)*zeta.s,lower.tail=!lower.tail))
	1 - sin(atant - 0.0184 * zeta.s * sqrt(df1 - 1))
}
...
# qqplot;
qqplot(qhcut2(ppoints(length(hcuts)),n.stok,n.obs,zeta.s),hcuts,
			 xlab = "Theoretical Approximate Quantiles", ylab = "Sample Quantiles")
qqline(hcuts,datax=FALSE,distribution = function(p) { qhcut2(p,n.stok,n.obs,zeta.s) },
			 col=2)
@
