...
one can imagine a set of two zero-mean assets, and another set of
two assets with positive mean, where the differences in the covariances
on the two sets implies that the sample optimal constrained \txtSR of the
first set is likely to be larger than that of the second. 
