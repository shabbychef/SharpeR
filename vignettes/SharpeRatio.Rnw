...
Here I describe a way to think about the multivariate Gaussian
distribution that eliminates, to some degree, the 
distinction between mean and covariance, in order to simplify
calculations and exposition. The basic idea is to append a 
deterministic 1 to the end of the random vector, then perform
inference on the uncentered second moment matrix of the appended
Gaussian.
...
$$
This relationship holds for the sample analogue as well:
$$
...
\begin{equation}
\ichol{\pvsm} = \twobytwo{\ichol{\pvsig}}{-\ichol{\pvsig}\pvmu}{0}{1},
\label{eqn:ichol_pvsm_nb}
\end{equation}
where $\ichol{\Mtx{A}}$ is the (upper triangular) Cholesky factor of the
inverse of \Mtx{A}.
