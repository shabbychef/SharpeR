...
% 2FIX: precompute this like the others, so you can have 2048
...
% 2FIX: start here, and talk more about the above MC sims and such ...
...
fit.df <- data.frame(apx=c(fit.med,fit.mean,fit.sd),
	mc=c(mc.med,mc.mean,mc.sd))
rownames(fit.df) <- c("median","mean","standard deviation")

# 2FIX: start here;yy
xres <- xtable(fit.df,label="tab:hcutfit",
	caption="Haircut fits ...")
print(xres,include.rownames=TRUE,sanitize.text.function=function(x){x})
