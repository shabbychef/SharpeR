...
to have $\psnr = 1.0\yrto{-1/2}$. I also draw from a symmetrized S \& P 500 returns
series. 
...
All distributions have been rescaled to have $\psnr = 1.0\yrto{-1/2}$; 
that is,
...
tests, against the alternative $H_a: \psnr < 1.0\yrto{-1/2}$.
...
xres <- xtable(res,label="tab:sharpe_skew_robustness",
							 ...
							 align="ll|lrr|rr",
							 caption=paste("Empirical type I rates of the test for $\\psnr = 1.0\\yrto{-1/2}$ via distribution of the \\txtSR are given for various distributions of returns.  The empirical rates are based on ", 
														 ntrials, 
...
This section is cribbed from Murphy, adapted for inference on 
the SNR. \cite{murphybayes}
