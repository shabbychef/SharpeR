...
is the \emph{excess} kurtosis of the returns distribution. These are
both zero for normally distributed returns, and so Mertens' form reduces
to Lo's form. \cite{mertens2002comments,Opdyke2007,sref2011}
These are unknown in practice, and have to be estimated.
2FIX.
...
existed for two years. Is this enough data assuming the SNR is 2.0 \yrto{-1/2}?
...
one sample \tstat-test.  Assuming the SNR of the process is 1.5 \yrto{-1/2}, what is the probability of a type II error?
\end{compactenum}
...
\Sexpr{foo.power$zeta}\yrto{-1/2} (a value which 
...
%fund is around $0.8\,\mbox{years}^{-0.5}$,'' or ``the Sharpe ratio of this stock over the past year is
%$0.4\,\mbox{months}^{-0.5}$.''
...
To demonstrate this fact, I look at the empirical type I rate for the hypothesis test: $H_0: \psnr = 1.0$
versus the alternative $H_1: \psnr > 1.0$ for different distributions of returns: 
...
different 2FIX. \cite{2010arXiv1010.2265G,2009arXiv0912.4554G,LambertW} 
...
to have $\psnr = 1.0$. I also draw from a symmetrized S \& P 500 returns
series.
The \tstat- and Tukey distributions are fairly kurtotic, but have zero
skew, while the Lambert W x Gaussian is skewed and (therefore) kurtotic.  
At the nominal $\typeI = 0.05$ level, we expect to get a reject rate around five percent.
...
xres <- xtable(res,label="tab:sharpe_skew_robustness",
							 ...
							 caption=paste("Empirical type I rates of the test for $\\psnr = 1.0$ via distribution of the Sharpe ratio are given for various distributions of returns.  The empirical rates are based on ", 
														 ntrials, 
														 " simulations of three years of daily returns,
with a nominal rate of $\\typeI = 0.05$. The 'corrected' type I rates refer to
a normal approximation using Mertens' correction. Skew appears to have a much more adverse effect than kurtosis alone."))
print(xres,include.rownames=FALSE)
@
%UNFOLD

The results are given in \tabref{sharpe_skew_robustness}, and indicate that skew is a more serious problem than
kurtosis. The results from the S \& P 500 data are fairly encouraging: broad market returns are perhaps not as skewed 
as the Lambert W x Gaussian distributions that we investigate, and the type I rate is near nominal when using three
years of data. Lumley \etal found similar results for the \tstat-test when looking at sample sizes greater than 
500. \cite{Lumley:2002} Since the \tstat-test statistic is equivalent to 
\txtSR (up to scaling), this result
carries over to the test for SNR.
...
$$\reti[t] = \pregco[0] 1 + \pregco[\smbox{HM}] \abs{\retk[\smbox{M},t]} + \perr[t],$$ 
where \retk[\smbox{M},t] are the returns of `the market' the portfolio is putatively timing. \cite{Henriksson_Merton_1981}
Actually, one or several factor timing terms can be added to any factor model.
Note that unlike the factor returns in models discussed above, one expects $\abs{\retk[\smbox{M}]}$ to have 
...
\begin{equation}
\tstat = \frac{\tr{\sregco}\convec - \contar}{\ssig \sqrt{\qform{\minv{\left(\gram{\mretk}\right)}}{\convec}}}.
\end{equation}
This statistic should be distributed as a non-central \tstat-distribution with non-centrality parameter
$$
\nctp = \frac{\tr{\pregco}\convec - \contar}{\psig \sqrt{\qform{\minv{\left(\gram{\mretk}\right)}}{\convec}}},
$$
...
\begin{equation}
\tstat = \frac{\tr{\sregco}\convec}{\ssig \sqrt{\qform{\minv{\left(\gram{\mretk}\right)}}{\convec}}}.
\end{equation}
This statistic should be distributed as a non-central \tstat-distribution with non-centrality parameter
$$
\nctp = \frac{\contar}{\sqrt{\qform{\minv{\left(\gram{\mretk}\right)}}{\convec}}},
$$
and $\ssiz - \nattf$ degrees of freedom. Thus we reject the null if \tstat is greater than 
...
\cite{rao2010linear,linmod2007,Kutner2004Applied,BDReg1993,kennedy2008guide}
...
$$\powr = 1 - \ncfcdf{\fqnt{1-\typeI}{\nlatf,\ssiz - \nlatf}}{\nlatf,\ssiz - \nlatf}{\ncfp[1]},$$
where 
$$\ncfp[1] = \ssiz\qform{\minv{\pvsig}}{\wrapNeParens{\pvmu[1] - \pvmu[0]}}$$
...
#foo <- data.frame("inter" = c(mod25$coefficients[1],mod50$coefficients[1],mod80$coefficients[1]),
...
foo <- data.frame("numerator" = c(
									...
									row.names = c("power = 0.25","power = 0.50","power = 0.80"))
xres <- xtable(foo,label="tab:htestpower",caption="The numerator in the sample size relationship required to achieve a fixed power in Hotelling's test is shown. The type I rate is 0.05.")
...
\section{Gaussian in Appended Form}%FOLDUP

\providecommand{\pvsm}[1][{}]{\mathSUB{\Mtx{\Theta}}{#1}}
\providecommand{\svsm}[1][{}]{\mathSUB{\Mtx{\hat{\Theta}}}{#1}}

Let \avreti be the \nlatf-variate vector \vreti appended with a 1:
$\avreti = \asvec{\tr{\vreti},1}$. Consider the second
moment of \avreti:
$$
\pvsm = \E{\ogram{\avreti}} = \twobytwo{\pvsig + \ogram{\pvmu}}{\pvmu}{\tr{\pvmu}}{1}.
$$
By inspection one can confirm that the
inverse of \pvsm is
$$
\minv{\pvsm} 
= \twobytwo{\minv{\pvsig}}{-\minv{\pvsig}\pvmu}{-\tr{\pvmu}\minv{\pvsig}}{1 + \qform{\minv{\pvsig}}{\pvmu}} 
= \twobytwo{\minv{\pvsig}}{-\pportwopt}{-\tr{\pportwopt}}{1 + \psnrsqopt}.
$$
Note that this relationship holds for the sample analogue as well:
$$
\minv{\twobytwo{\svsig + \ogram{\svmu}}{\svmu}{\tr{\svmu}}{1}} 
= \twobytwo{\minv{\svsig}}{-\sportwopt}{-\tr{\sportwopt}}{1 + \ssrsqopt}.
$$
The Cholesky factor of \minv{\pvsm} is
$$
\ichol{\pvsm} = \twobytwo{\trichol{\pvsig}}{-\trichol{\pvsig}\pvmu}{0}{1}.
$$

Note that
$$
\qform{\minv{\pvsig}}{\wrapParens{\vreti - \pvmu}} = 
\qform{\minv{\pvsm}}{\avreti} - 1.
$$
Using the block determinant formula, we find that \pvsm has the same 
determinant as \pvsig: $\det{\pvsm} = \det{\pvsig}.$  Then if
\vreti is multivariate Gaussian, then the density of \vreti can be
expressed more simply as
\begin{equation*}
\begin{split}
\normpdf{\vreti}{\pvmu,\pvsig} &= \frac{1}{\sqrt{\wrapParens{2\pi}^{\nlatf}\det{\pvsig}}} 
\longexp{-\half \qform{\minv{\pvsig}}{\wrapParens{\vreti - \pvmu}}},\\
 &= \frac{\wrapParens{\det{\pvsig}}^{-\half}}{\wrapParens{2\pi}^{\nlatf/2}}
\longexp{-\half \wrapParens{\qform{\minv{\pvsm}}{\avreti} - 1}},\\
...
\end{equation*}
Now note that $\gram{\amreti}/\ssiz$ is a sample estimate of 
\pvsm, call it \svsm. Then 
\begin{equation*}
...
\pvsm and finding a root.
That is,
\begin{equation*}
\begin{split}
0 &= \drbydr{\log\FOOpdf{}{\svsm}{\pvsm}}{\pvsm},\\
	&= - \tr{\pvsm[m]} + \tr{\svsm},
\end{split}
\end{equation*}
leaning heavily on the Matrix Cookbook for 
the derivatives. \cite{petersen2006matrix}
This is solved by $\pvsm[m] = \svsm$, and thus the
na\"{i}ve sample estimate is the MLE.
%UNFOLD

\subsection{Likelihood Ratio Test}%FOLDUP

Now suppose you wish to compare two possible values of the population
parameter, \pvsm[1] and \pvsm[2], based on their likelihoods. The
difference in log likelihoods is
\begin{equation}
\begin{split}
\log\wrapParens{\frac{\FOOpdf{}{\svsm}{\pvsm[1]}}{\FOOpdf{}{\svsm}{\pvsm[2]}}}
&= -\half[\ssiz] \wrapParens{\logdet{\pvsm[1]} - \logdet{\pvsm[2]} +
\trace{\minv{\pvsm[1]}\svsm} - \trace{\minv{\pvsm[2]}\svsm}},\\
&= -\half[\ssiz] \wrapParens{\logdet{\pvsm[1]\minv{\pvsm[2]}} + 
\trace{\wrapBracks{\minv{\pvsm[1]} - \minv{\pvsm[2]}}\svsm}}.
\end{split}
\end{equation}
...
&= - \tr{\pvsm[m]} + \tr{\wrapParens{\svsm - \sum_i \lambda_i \lrtA[i]}},\\
\therefore \pvsm[m] &= \svsm - \sum_i \lambda_i \lrtA[i].
\end{split}
\end{equation*}
The maximum likelihood estimator under the constraints has to be
found numerically by solving for the $\lambda_i$.
