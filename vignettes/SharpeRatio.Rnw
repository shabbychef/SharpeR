...
leaning heavily on the Matrix Cookbook for 
the derivatives. \cite{petersen2006matrix}
This is solved by $\pvsm[\txtMLE] = \svsm$, and thus the
na\"{i}ve sample estimate is the MLE.
%UNFOLD
...
\drbydr{\trace{\lrtA[i]\minv{\pvsm}}}{\pvsm},\\
&= - \tr{\pvsm[\txtMLE]} + \tr{\wrapParens{\svsm - \sum_i \lambda_i \lrtA[i]}},\\
...
vector of $\lambda_i$. The residual of the \kth{k} estimate, \vitrlam{k}{} is
\begin{equation}
\vitrerr{k}{i} \defeq 
...
\end{equation}
Once the optimum is found, one can compute the difference in log
likelihoods under the null..

one can employ Wilks' Theorem ...
\cite{anderson2003introduction,Bilodeau1999}
