...
\providecommand{\ssrsqoptp}[1]{\ssrsq[*,#1]}
...
\end{equation}
where $\Mtx{G}$ is an $n_g \times \nstrat$ matrix of rank $n_g$, and,
...
$n_g$ rows of the identity matrix, and \vect{g} is the zero vector;
in this case, feasible portfolios are `hedged' with respect 
to the $n_g$ assets selected by \Mtx{G}
...
\begin{equation}
\ssrsqopt = \qform{\minv{\svsig}}{\svmu} -
	\qform{\minv{\wrapParens{\qoform{\svsig}{\Mtx{G}}}}}{\wrapParens{\Mtx{G}\svmu}}.
\label{eqn:ssr_Gcons}
\end{equation}
...
These questions are related to the known tests 
of \emph{portfolio spanning}.  \cite{KanZhou2012,BrittenJones1999,HKspan1987}
