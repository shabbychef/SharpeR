...
\end{equation}
where \pvmom[i] is the uncentered \kth{i} moment of \reti.

When returns are assumed to be normal,
the skew and excess kurtosis are known to be zero, so \pvvar takes
the form
$$
\pvvar = \twobytwossym{\pvmom[2] -
\pvmu^2}{2\mu\wrapParens{\pvmom[2]-\pvmu^2}}{2\wrapParens{\pvmom[2]^2 -
\pvmu^4}}.
$$
This reduces to Lo's form of the
