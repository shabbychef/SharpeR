...
The technique outlined above admits a number of interesting extensions.

\subsection{Hedging Constraint} % constrained case ... %FOLDUP
...
\begin{equation*}
\Delhej\minv{\pvsm} =
\twobytwo{ \qiform{\pvsig}{\pvmu} - \wrapProj{\pvsig}{\hejG} }{
...
The statistic $\qiform{\pvsig}{\pvmu} - \wrapProj{\pvsig}{\hejG}$, for the
...
\begin{theorem}%FOLDUP
Let \svsm be the unbiased sample estimate of 
\pvsm, based on \ssiz \iid samples of \vreti. Let $\Delhej\minv{\pvsm}$
...
$$
Via \lemmaref{deriv_vech_matrix_inverse}, and the proof of 
\theoremref{inv_distribution}, it suffices to prove that
$$
\dbyd{\fvec{\wrapProj{\pvsm}{\hejGt}}}{\fvec{\pvsm}} = 
-\wrapParens{\AkronA{\tr{\hejGt}}} \wrapParens{\AkronA{\minv{\wrapParens{\qoform{\pvsm}{\hejGt}}}}}
\wrapParens{\AkronA{\hejGt}}.
$$
...
\fvec{\Mtx{B}},\quad\mbox{and, therefore,}\quad
\dvecbydvec{\Mtx{A}\Mtx{B}\Mtx{C}}{\Mtx{B}} = \Mtx{A}\kron\tr{\Mtx{C}}.
$$
...
\dbyd{\fvec{\wrapProj{\pvsm}{\hejGt}}}{\fvec{\pvsm}} 
&=
\dvecbydvec{\wrapProj{\pvsm}{\hejGt}}{\minv{\wrapParens{\qoform{\pvsm}{\hejGt}}}} 
\dvecbydvec{\minv{\wrapParens{\qoform{\pvsm}{\hejGt}}}}{\qoform{\pvsm}{\hejGt}}
\dvecbydvec{\qoform{\pvsm}{\hejGt}}{\pvsm}\\
&= 
...
One possible deficiency of the above analysis is that it ignores
`voltility clustering'. \cite{stylized_facts} To deal with this,
consider a scalar volatility indicator, \fvola[i]. This is a random
variable which is observable at the time the investment decision is
required to capture \vreti[i+1].
%UNFOLD

\subsection{Conditional Expectation}%FOLDUP
...
This is a rather tedious computation; here I present a session in Sympy
\cite{sympy} that performs this computation:
