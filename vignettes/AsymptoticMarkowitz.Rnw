...
The script for each of these extensions is the same: define, then solve,
some portfolio optimization problem; show that the solution can be defined
in terms of some transformation of $\minv{\pvsm}$; implicitly the sample
analogue is defined in terms of the same transformation of $\minv{\svsm}$; 
...
This is a rather tedious computation; here I present an iPython
notebook using Sympy to perform this computation.
\cite{PER-GRA:2007,sympy} 
