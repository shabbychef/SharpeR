...
homoskedasticity. \cite{stylized_facts} To deal with this,
...
is $\fvola[i]\sqrt{\qiform{\pvsig}{\pvmu}}$, thus the names.

The optimal portfolio under both models is the same, as stated
in the following lemma, the proof of which follows exactly as that of 
...
based on \ssiz \iid samples of \asvec{\fvola,\vreti}.
...
\fvola[i]^{-2} \pvsig,
\end{align}
where \pRegco is some $\nlatf \times \nfac$ matrix. By letting
$\vfact[i] = \asvec{\fvola[i]^{-1},1}$ we recover 
the mixed model in \eqnref{cond_model_III}. However, this model is
considerably more general.  The conditionally-optimal portfolio
...
{\bf Caution:} It is emphatically \emph{not} the case that 
