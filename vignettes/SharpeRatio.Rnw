...
Let \pvsnr be the vector of SNR of the assets. We have
$$
\pvsnr[i] = \frac{\pvmu[i]}{\sqrt{\pvmom[2,i] - \pvmu[i]^2}}.
$$
...
\svmom[2]: 
$\svsr[i] = \fracc{\svmu[i]}{\sqrt{\svmom[2,i] - \svmu[i]^2}}.
$
...
\onebytwo{\diag{\frac{\pvmom[2]}{\wrapParens{\pvmom[2] - \pvmu^2}^{3/2}}}}{ 
\diag{\frac{-\pvmu}{2 \wrapParens{\pvmom[2] - \pvmu^2}^{3/2}}}}.
\label{eqn:sr_deriv}
\end{equation}
...
2FIX: these need to be filled out.

For the $\nlatf=1$ case, when the returns are assumed to be normal,
the matrix \pvvar is diagonal. This reduces to Lo's form of the
standard error, \eqnref{srse_lo}.
For the $\nlatf=1$ case when returns are not assumed to be normal,
if one takes the vanilla sample estimate of \pvvar, one arrives
at Mertens' form of the standard error,
\eqnref{srse_mertens}. 2FIX: verify these are true!



%UNFOLD

\subsection{Tests of equality of multiple \txtSR}%FOLDUP


\cite{Ledoit2008850,Leung2008,Wright2012} 
%UNFOLD
