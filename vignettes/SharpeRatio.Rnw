...
see \figref{sr_bias}.  However, there are situations in which it might be unacceptably large. 
...
\ssrsqoptG{\hejG}}{\frac{\ssiz - 1}{\ssiz} + \ssrsqoptG{\eye}}
\label{eqn:giri_lrt}
\end{equation}
...
\psnrsqoptG{\hejG}}{1 + \frac{\ssiz}{\ssiz-1}\ssrsqoptG{\eye}}}.
$$
...
In section 3, Giri claims that $Z$ takes a (non-central) beta distribution. 
From inspection, it is a 'type II' non-central beta, which can be
transformed into a noncentral \flaw{}:
\begin{equation}
\frac{N-FIX}{FIX} \frac{1-Z}{Z} = FIX \frac{\GN\GSR{2} - \GN\GSR{1}}{1 + \GN\GSR{1}}.
\end{equation}
...
assets. In the terminology of the rest of the paper, 
$$
\GN\GSR{i} = \frac{\ssiz}{\ssiz-1}\ssrsqoptG{i}
$$
