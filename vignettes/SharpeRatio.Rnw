...
Van Belle suggests one consider, in priority order, assumptions of independence,
...
Here I will consider the case heteroskedastic asset returns with constant
SNR. That is, both the volatility and expected return are changing over 
time, with their ratio constant. 
Suppose that \levi[i] and \reti[i] are independent random variables with
...
and a `Lambert W x Gaussian' distribution, with different .  \cite{2010arXiv1010.2265G,2009arXiv0912.4554G,LambertW} 
...
<<'skewstudy',echo=FALSE,results='asis'>>=
...
moment of \avreti:
$$
\Mtx{L} = \E{\ogram{\avreti}} = \twobytwo{\pvsig + \ogram{\pvmu}}{\pvmu}{\tr{\pvmu}}{1}.
$$
...
$$
The Cholesky factor of \minv{\Mtx{L}} is
$$
\ichol{\Mtx{L}} = \twobytwo{\trichol{\pvsig}}{-\trichol{\pvsig}\pvmu}{0}{1}.
$$

Note that
$$
\qform{\minv{\pvsig}}{\wrapParens{\vreti - \pvmu}} = 
\qform{\minv{\Mtx{L}}}{\avreti} - 1.
$$
Using the block determinant formula, we find that \Mtx{L} has the same 
determinant as \pvsig: $\det{\Mtx{L}} = \det{\pvsig}.$  Then if
...
\longexp{-\half \wrapParens{\qform{\minv{\Mtx{L}}}{\avreti} - 1}},\\
 &= \wrapParens{2\pi}^{-\nlatf/2} \wrapParens{\det{\Mtx{L}}}^{-\half}
\longexp{-\half \wrapParens{\qform{\minv{\Mtx{L}}}{\avreti} - 1}},\\
 &= \wrapParens{2\pi}^{-\nlatf/2}
\longexp{\half - \half \logdet{\Mtx{L}} - \half \trace{\minv{\Mtx{L}}\ogram{\avreti}}},\\
- \log\normpdf{\vreti}{\pvmu,\pvsig} &= 
  c_{\nlatf} 
+ \half \logdet{\Mtx{L}} 
+ \half \trace{\minv{\Mtx{L}}\ogram{\avreti}},
\end{split}
\end{equation*}
for the constant 
...
- \log\normpdf{\amreti}{\Mtx{L}} =
  \ssiz c_{\nlatf} 
+ \half[\ssiz] \logdet{\Mtx{L}} 
+ \half \trace{\minv{\Mtx{L}}\gram{\amreti}}.
\end{equation*}
Now note that $\gram{\amreti}/\ssiz$ is a sample estimate of 
\Mtx{L}, call it \Mtx{\hat{L}}. Then 
\begin{equation*}
\frac{- 2\log\normpdf{\amreti}{\Mtx{L}}}{\ssiz} = 
  c_{\nlatf} 
+ \logdet{\Mtx{L}} 
+ \trace{\minv{\Mtx{L}}\Mtx{\hat{L}}}.
\end{equation*}

By Lemma (5.1.1) of Press \cite{press2012applied}, this can be expressed
as a density on \Mtx{\hat{L}}:
\begin{equation*}
\begin{split}
\frac{- 2\log\FOOpdf{}{\Mtx{\hat{L}}}{\Mtx{L}}}{\ssiz} 
&= \frac{- 2\log\normpdf{\amreti}{\Mtx{L}}}{\ssiz}\\
&\phantom{=}\,
	-\frac{2}{\ssiz}\wrapParens{\half[\ssiz-\nlatf-1]\logdet{\Mtx{\hat{L}}} +
	\half[\nlatf]\wrapParens{\ssiz - \half[\nlatf] + \half} \log\pi -
	\sum_{j=1}^{\nlatf} \log\funcit{\Gamma}{\half[\ssiz +1-j]}},\\
&= c_{\nlatf} - \frac{\nlatf}{\ssiz}\wrapParens{\ssiz - \half[\nlatf] + \half} \log\pi 
	- \frac{2}{\ssiz} \sum_{j=1}^{\nlatf}
	\log\funcit{\Gamma}{\half[\ssiz +1-j]}\\
&\phantom{=}\,
	+ \logdet{\Mtx{L}} - \frac{\ssiz-\nlatf-1}{\ssiz}\logdet{\Mtx{\hat{L}}}
	+ \trace{\minv{\Mtx{L}}\Mtx{\hat{L}}},\\
&= c'_{\ssiz,\nlatf} 
	+ \logdet{\Mtx{L}} - \frac{\ssiz-\nlatf-1}{\ssiz}\logdet{\Mtx{\hat{L}}}
	+ \trace{\minv{\Mtx{L}}\Mtx{\hat{L}}}.
