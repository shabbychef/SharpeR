...
\subsection{Portfolio optimization with an \ellk{2} constraint}

\label{sec:insane_risk_manager}

Consider the constrained portfolio optimization problem on \nstrat assets,
\begin{equation}
\max_{\sportw : \gram{\sportw} \le \Rbuj^2}
\frac{\trAB{\sportw}{\svmu} - \rfr}{\sqrt{\qform{\svsig}{\sportw}}},
\label{eqn:cons_port_ell2}
\end{equation}
where \Rbuj is an \ellk{2} constraint. 
...
\providecommand{\daset}[1]{\mathSUB{\mathcal{C}{#1}}}
...
Fix the one-parameter family of distribution on \retj. Then, 
for given \dknok, \dprob, and \ssiz, the acceptable funds are 
defined by the set
\begin{equation}
\daset{\dknok,\dprob,\ssiz} \defeq \setwo{\wrapParens{\psig,\psnr}}{\Pr{\frac{\ddown{\ssiz}}{\psig} \ge \dknok} \le \dprob}.
\end{equation}
It is obvious that the set \daset{\dknok,\dprob,\ssiz} has certain
`monotonicity' properties: a fund with lower volatility or higher \txtSNR
than a fund in the set is also in the set. That is, 
\begin{equation*}
%\begin{split}
\wrapBracks{\wrapParens{\psig[2],\psnr} \in \daset{\dknok,\dprob,\ssiz}}
\bigwedge
\wrapBracks{\psig[1] < \psig[2]}
\Rightarrow
\wrapParens{\psig[2],\psnr} \in \daset{\dknok,\dprob,\ssiz},
%\wrapBracks{\wrapParens{\psig,\psnr} \in \daset{\dknok,\dprob,\ssiz}}
%\bigwedge
%\wrapBracks{\psnr < \psnr}
%&\Rightarrow
%\wrapParens{\psig[2],\psnr} \in \daset{\dknok,\dprob,\ssiz}.
%\end{split}
\end{equation*}
