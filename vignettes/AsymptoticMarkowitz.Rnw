...
\subsection{The \txtMP maximizes the \txtSR}%FOLDUP

\begin{lemma}[\txtSR optimal portfolio]
Assuming \pvmu is not the vector of zeros, and \pvsig is invertible,
...
\end{equation}
where $\psnrsqopt = \qiform{\pvsig}{\pvmu}.$ The optimimum
...
where 
$$
\Mtx{K} = - \Rbuj \asvec{\half \frac{\pportwoptR}{\psnropt},
\oneby{\psnropt}\eye[\nlatf],\mzero},
$$
