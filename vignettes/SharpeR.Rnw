...
nco <- 20
nrep <- 1000
afoo <- replicate(nrep,sr_equality_test(matrix(rnorm(nro*nco),ncol=nco),type="F"))
vvv <- unlist(afoo["p.value",])
qqunif(vvv)

# howzabout heteroskedasticity?
afoo <- replicate(nrep,sr_equality_test(matrix(rt(nro*nco,df=4),ncol=nco),type="F"))
vvv <- unlist(afoo["p.value",])
qqunif(vvv)
@
...
DJIA <- getSymbols(Symbols='DJIA',src='FRED',auto.assign=FALSE)
DJIA <- DJIA[!is.na(DJIA)]

DJIA.lr <- diff(log(DJIA))
DJIA.lr <- DJIA.lr[-1]

AAPL <- getSymbols(Symbols='AAPL',src='yahoo',from='1995-01-01',auto.assign=FALSE)
AAPL <- AAPL[,"AAPL.Adjusted"]
AAPL <- AAPL[!is.na(AAPL)]

AAPL.lr <- diff(log(AAPL))
AAPL.lr <- AAPL.lr[-1]

t.inter <- function(xTEO,yTEO) {
	xTEO[xTEO %in% yTEO]
}

foo.time <- t.inter(time(AAPL.lr),time(DJIA.lr))

blah <- cbind(AAPL.lr[foo.time,],DJIA.lr[foo.time,])

sr_equality_test(blah)
sr_equality_test(blah,type="F")
