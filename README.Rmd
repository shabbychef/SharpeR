...
```{r real_data,dependson=c('stock_loading'),eval=TRUE,echo=TRUE}
...
```{r unpaired_null,eval=TRUE,echo=TRUE}
set.seed(9001)
pvals <- replicate(1e4L, {
...
ph <- ggplot(data, aes(sample = pvals)) + 
...
```{r unpaired_null_two,eval=TRUE,echo=TRUE}
set.seed(9002)
pvals <- replicate(1e4L, {
	zeta <- 0.1
	sg <- 0.01
	X <- matrix(rnorm(500*6,mean=zeta*sg,sd=sg),ncol=6)
	inp <- as.sr(X)
	etc <- sr_unpaired_test(inp,contrasts=rep(1,dim(X)[2]),null.value=dim(X)[2]*zeta)
	etc$p.value
})
require(ggplot2)
...
```{r ff_momentum_check,eval=TRUE,echo=TRUE}
library(xts)
if (!require(tsrsa)) {
...
etc <- sr_unpaired_test(list(sr.jan,sr.rem),contrasts=c(1,-1),null.value=0,ope=12)
print(etc)
...
okvals <- replicate(500, {
	...
	is.ok <- (pint[,1] <= oos$sr) & (oos$sr <= pint[,2])
	is.ok
})
...
```{r ff_predint_check,eval=TRUE,echo=TRUE}
library(xts)
if (!require(tsrsa)) {
	devtools::install_github('shabbychef/tsrsa')
	library(tsrsa)
...
# shoot me if this is how to get the year number from a time index.
...
okvals <- lapply(unique(yrno),function(yr) { 
	...
	pint <- predint(sr.is,oosdf=sr.oos$df,oosrescal=sr.oos$rescal,ope=sr.oos$ope)
	is.ok <- (pint[,1] <= sr.oos$sr) & (sr.oos$sr <= pint[,2])
	is.ok
})
...
```{r ff_predint_check_shuf,dependson=c('ff_predint_check'),eval=TRUE,echo=TRUE}
