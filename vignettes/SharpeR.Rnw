...
\label{eqn:ssr_Gcons}
\end{equation}
using the notation from the previous section, and setting $\rfr=0$.

Some natural questions to ask are
\begin{enumerate}
\item Is the constrained portfolio `good'? More formally, does the optimal
constrained portfolio using the (unknown) population parameters have non-zero
\txtSR? Can we estimate how large it is?
...
The first question generalizes the known tests of \emph{portfolio spanning}.  
...
Unfortunately, the sample statistic \ssrsqoptG{+} is not a \emph{similar}
statistic.  \cite{Sen1999264} That is, it depends on the population 
analogue, \psnrsqoptG{+}, but also on the uknown nuisance 
parameter, \pvsig.  Using \ssrsqoptG{+} to test the
hypothesis $H_0: \psnrsqoptG{+} = 0$ only yields a conservative test, with
a maximal type I rate.
\nocite{Silvapulle1996137,Perlman_1969}

To a certain degree, this makes sense, since the  ...
...
(or my implementation thereof!) apparently reject the null 
...
% bibliography%FOLDUP
\nocite{CambridgeJournals:4493808,lo2002,Lecoutre2007,Johnson:1940,Ledoit2008850}
