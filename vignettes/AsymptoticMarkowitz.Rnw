...
\begin{equation}
\pportwopt \defeq \lambda \minvAB{\pvsig}{\svmu}
\end{equation}
...
\begin{equation}
\pportwopt = \argmax_{\pportw : \qform{\pvsig}{\pportw} \le \Rbuj^2} 
\frac{\trAB{\pportw}{\pvmu} - \rfr}{\sqrt{\qform{\pvsig}{\pportw}}},
\label{eqn:opt_port_I}
\end{equation}
where \rfr is the risk-free, or `disastrous', rate of return, and 
...
Okhrin and Schmid, and Bodnar and Okhrin give the (univariate) density of
...
not of \pvsig. \cite{chopra1993effect} 
...
The result then follows from the delta method.
\end{proof}
...
of \vreti and of \svsm in simple terms involving \nlatf, \pvsm, and \ssiz.
...
Thus the analogue of \lemmaref{inv_distribution} for Gaussian returns
